...
define(function (require, exports, module) {
    ...
    var Model = Backbone.Model.extend({
        constructor:function (attributes) {
            var schema = attributes.schema || this.schema;
            if (attributes.schema)
                delete(attributes.schema);
            if (schema)
                this.validator = new ModelValidator(schema);
    ...
    var ModelValidator = Backbone.Model.extend({
        tasksBuilder:new InterdependentTaskSeriesBuilder(tests),
        checks:checks,
        tests:tests,
        constructor:function (schema) {
            this.runners = {};
            this.attributeModels = {};
            _.each(schema, function (config, attribute) {
                var tasks = this.tasksBuilder.createTasks(config);
                _.each(tasks, function (task, key) {
                    if (key in this.checks)
                        this.checks[key].call(config, config[key], key);
                }, this);
                var runner = new AsyncSeriesTaskRunner(tasks, config);
                this.runners[attribute] = runner;
                var model = new Backbone.Model();
                var valid;
                runner.on("start", function (iterator) {
                    model.clear();
                    valid = true;
                }, this);
                runner.on("abort", function (iterator) {
                    model.clear();
                    valid = null
                }, this);
                runner.on("error", function (key, error, iterator) {
                    model.set(key, {error:error});
                }, this);
                runner.on("done", function (key, passed, iterator) {
                    model.set(key, passed);
                    if (!passed)
                        valid = false;
                }, this);
                runner.on("end", function (iterator) {
                    this.set(attribute, valid);
                }, this);
                this.attributeModels[attribute] = model;
            }, this);
            Backbone.Model.call(this);
        },
        run:function (attributes) {
            _.each(attributes, function (value, attribute) {
                if (!this.runners[attribute])
                    return;
                this.runners[attribute].run({
                    value:value
                });
    ...
    module.exports = {
        ...
        install:function (pack) {
            _.extend(tests, pack.tests);
            _.extend(checks, pack.checks);
        }
    };
});